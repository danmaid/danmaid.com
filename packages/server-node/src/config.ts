import { createResource } from '@danmaid/master'
...
export const config = createResource('/config', {
  key,
  cert,
  port: 443,
  listen: true,
  dir: './data',
  index: 'https://localhost/index.html',
})
config.addEventListener('error', (err) => {
  console.log('error in config. use built-in config.')
